<template>
  ...
  <div class="dialog-button-wrap">
    ...
    <el-button @click="editBanner(ruleFormRef)" type="success" :loading="subLoading" color="#083a6d">保存</el-button>
  </div>
</template>
...
import {editBannerApi} from "@/api/edu/banner/banner";
// 表单实例对象
...
const bannerInfo = ref(props.bannerInfo)
// 定义事件
const emit = defineEmits(['closeEditBannerForm','success'])
// 给表单填充数据
for (const key in formBanner) {
  formBanner[key] = bannerInfo.value[key]
}
/**
 * 保存轮播图
 * @param formEl
 */
const editBanner = async (formEl: FormInstance | undefined)=> {
  if(!formEl)return
  await formEl.validate(async (valid,fields)=> {
    subLoading.value = true
    if(valid){
      const { data } = await editBannerApi(formBanner)
      if(data.status === 200){
