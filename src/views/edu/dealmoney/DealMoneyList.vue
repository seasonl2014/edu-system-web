       ...
       <!--搜索区域 start-->
        <div class="card-search">
          <el-row :gutter="10">
            ...
            <el-col :span="6">
              <el-select v-model="buyType" class="m-2" placeholder="请选择">
                <el-option label="购买课程" value="0"/>
  ...
  <div class="table-box">
    ...
    >
      ...
      <el-table-column label="订单编号" show-overflow-tooltip>
        <template #default="scope">
          <el-tag type="success">{{scope.row.orderNo}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="成交类型">
        <template #default="scope">
          <el-tag v-if="scope.row.buyType===0">购买课程</el-tag>
          <el-tag type="warning" v-else>购买VIP</el-tag>
        </template>
      </el-table-column>
...
import {getListApi} from "@/api/edu/dealmoney/dealmoney";
...
const { total,tableData,pageSize,pageIndex,loading,searchValue,buyType}=toRefs(state)
</script>
