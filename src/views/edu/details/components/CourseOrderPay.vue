<template>
  ...
  <div class="edu-container pay-wrap">
   <el-card>
     <template #header>
       ...
       <div class="pay-title">你正在购买课程：{{orderInfo!=null?orderInfo.title:''}}</div>
     </template>
     <!--支付方式 start-->
     <div class="pay-way">
  ...
  <el-dialog
      ...
      :show-close="false"
      :title="(orderInfo!=null?orderInfo.title:'')+'（打开微信扫一扫）'" center v-model="payVisible" width="24%">
...
import {getCourseOrderInfoApi, payBuyCourseApi} from "@/api/edu/detail/detail";
...
const getOrderInfo = async ()=> {
  const { data } = await getCourseOrderInfoApi(orderNo.value)
